...
d_to_rgb = np.array([[9.9996518012567637e-01, 2.6765126468950343e-03, -7.9041012313000904e-03, -2.5558943178152542e-02],
                     ...
                     [7.8819942130445332e-03, 8.1718328771890631e-03, 9.9993554558014031e-01, 2.0318321729487039e-03], [0, 0, 0, 1]])

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Download and setup the 7Scenes dataset.', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument('--eye', type=str, choices=['none', 'calibrated'], default='none', help='none: ignore eye coordinates; original: calibrate original depth maps and precompute eye coordinates')
        ...
        def link_frames(split_file, variant):
            ...
            for seq in split:
                ...
                for img in images:
                    ...
                    os.system(f'cp {src_folder}/{ds}/{seq}/{img} {rgb_path}/{seq}-{img}')
                ...
                if opt.poses == 'original':
                    ...
                    for p_file in pose_files:
                        ...
                        os.system(f'cp {src_folder}/{ds}/{seq}/{p_file} {poses_path}/{seq}-{p_file}')
                elif opt.poses == 'pgt':

                    # use poses as externally provided
                    for p_file in pose_files:
                        cam_pose, _ = pgt_poses[os.path.join(seq, dutil.get_base_file_name(p_file))]
                        dutil.write_cam_pose(f"{poses_path}/{seq}-{p_file}", cam_pose)
                ...
                if opt.poses == 'pgt':
                    ...
                    for p_file in pose_files:
                        ...
                        dutil.write_focal_length(target_folder + variant + '/calibration/' + seq + '-' + calib_file, rgb_f)
                else:
                    for i in range(len(images)):
                        dutil.write_focal_length(target_folder + variant + '/calibration/%s-frame-%s.calibration.txt' % (seq, str(i).zfill(6)), focal_length)
                ...
                for d_file in depth_files:
                    ...
                    torch.save(eye_tensor, target_folder + variant + '/eye/' + seq + '-' + d_file[:-10] + '.eye.dat')

        link_frames('TrainSplit.txt', 'train')
        link_frames('TestSplit.txt', 'test')

    Parallel(n_jobs=7, verbose=0)(map(delayed(process_scene), ['chess', 'fire', 'heads', 'office', 'pumpkin', 'redkitchen', 'stairs']))
