...
class ACEVisualizer:
    ...
    def _generate_camera_pan(self, pan_number_cams, mapping_poses, pan_angle_coverage):
        """
    ...
    def _generate_camera_trajectory(self, mapping_poses, mapping_image_paths):
        ...
        for frame_idx in range(len(mapping_poses)):
            ...
            self.trajectory_buffer.add_camera_frustum(frustum_pose, image_file=mapping_image_paths[frame_idx], sparse=True)
    ...
    def setup_mapping_visualisation(self, pose_files, image_files, frame_count, camera_z_offset):
        """
        ...
        self.pan_cams = self._generate_camera_pan(pan_number_cams=frame_count + self.framecount_transition, mapping_poses=mapping_poses, pan_angle_coverage=self.pan_angle_coverage)

        # reset camera trajectory to render
        self.trajectory_buffer = vutil.CameraTrajectoryBuffer(frustum_skip=self.mapping_frustum_skip, frustum_scale=self.frustum_scale_mapping)
    ...
    def _draw_pose_error_hist(self, image, errors):
        ...
        hist_values[:self.err_hist_bins - 1], _ = np.histogram(errors, bins=self.err_hist_bins - 1, range=(0, self.reloc_vis_error_threshold))
    ...
    def finalize_mapping(self, network, data_loader):
        ...
        for transition_idx in range(self.framecount_transition):
            ...
            # update rolling buffer
            self.point_cloud_buffer.update_buffer(main_pc_xyz_buffer[transition_idx], main_pc_clr_buffer[transition_idx])
        ...
        state_dict = {'map_xyz': map_xyz, 'map_clr': map_clr, 'frame_idx': self.frame_idx, 'camera_buffer': self.scene_camera.get_camera_buffer()}
    ...
    def setup_reloc_visualisation(self, frame_count, network, data_loader, camera_z_offset, reloc_frame_skip):
        ...
        try:
            ...
            self.scene_camera = vutil.LazyCamera(backwards_offset=camera_z_offset, camera_buffer=state_dict['camera_buffer'])
        except:
    ...
    def render_reloc_frame(self, query_pose, query_file, est_pose, est_error, sparse_query):
        ...
        if self.reloc_buffer_previous_est is not None:
            self.trajectory_buffer.add_position_marker(marker_pose=self.reloc_buffer_previous_est, marker_color=self.reloc_buffer_previous_clr, marker_extent=marker_size)
